...
interface QuickExitLine { uid: number; itemId: number | null; itemName: string; unitOfMeasure: string | null; quantity: number; unitValue: number | null; notes: string; }
...
export class QuickExitComponent implements OnInit {
  ...
  get totalItems(): number { return this.lines.filter(line => line.itemId || line.itemName).length; }
  ...
  get purchaseList(): QuickExitLine[] { return this.lines.filter(line => line.itemId || line.itemName).filter(line => this.lineStatus(line) !== 'DISPONIVEL'); }
  ...
  availableToExit(line: QuickExitLine): number { const p=this.selectedProduct(line); if(!p)return 0; return Math.min(Number(line.quantity||0), p.currentQuantity||0); }
  missingQuantity(line: QuickExitLine): number { const p=this.selectedProduct(line); return Math.max(0, Number(line.quantity||0)-(p?.currentQuantity||0)); }
  lineStatus(line: QuickExitLine): string { const p=this.selectedProduct(line); if(!p)return 'NAO_CADASTRADO'; if((p.currentQuantity||0)>=Number(line.quantity||0))return 'DISPONIVEL'; if((p.currentQuantity||0)>0)return 'ESTOQUE_INSUFICIENTE'; return 'PRECISA_COMPRAR'; }
  statusText(s:string):string{return{DISPONIVEL:'Disponível',ESTOQUE_INSUFICIENTE:'Estoque insuficiente',PRECISA_COMPRAR:'Precisa comprar',NAO_CADASTRADO:'Não cadastrado'}[s]||s;}
  ...
  private newLine(): QuickExitLine { return {uid:this.uid++,itemId:null,itemName:'',unitOfMeasure:null,quantity:1,unitValue:null,notes:''}; }
