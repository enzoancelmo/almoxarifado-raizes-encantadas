...
export class StockMovementFormComponent implements OnInit{
 ...
 readonly form=this.fb.group({productId:[null as number|null,Validators.required],movementType:[null as StockMovementType|null,Validators.required],quantity:[null as number|null,[Validators.required,Validators.min(0)]],reason:['',[Validators.maxLength(255)]]});
 ...
 ngOnInit():void{this.productsService.list().pipe(timeout(8000),finalize(()=>{this.loading=false;this.cdr.markForCheck();})).subscribe({next:data=>{this.products=data;const selected=this.route.snapshot.queryParamMap.get('productId');if(selected)this.form.patchValue({productId:Number(selected)});},error:()=>this.error='Não foi possível carregar os produtos. Confirme se o backend está ativo em http://localhost:8080.'});}
 ...
 save():void{if(this.form.invalid){this.form.markAllAsTouched();return;}const value=this.form.getRawValue();const payload:StockMovementPayload={productId:Number(value.productId),movementType:value.movementType!,quantity:Number(value.quantity),reason:value.reason?.trim()||null};this.saving=true;this.error='';this.movements.create(payload).subscribe({next:()=>void this.router.navigate(['/movimentacoes'],{queryParams:{success:'Movimentação registrada e estoque atualizado.'}}),error:r=>{this.error=r.error?.message||'Não foi possível registrar a movimentação.';this.saving=false;this.cdr.markForCheck();}});}
