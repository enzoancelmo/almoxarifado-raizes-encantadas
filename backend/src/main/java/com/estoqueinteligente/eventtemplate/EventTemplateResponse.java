...
public class EventTemplateResponse {
    ...
    public EventTemplateResponse(EventTemplate t){id=t.getId();name=t.getName();description=t.getDescription();eventType=t.getEventType();active=t.isActive();notes=t.getNotes();createdAt=t.getCreatedAt();updatedAt=t.getUpdatedAt();items=t.getItems().stream().sorted(Comparator.comparing(i->i.getProduct().getName())).map(EventTemplateItemResponse::new).toList();}
